 ...
 ************************************/
const projectId = '1e376e78c0aa32bd807ec395c7f7217e';
const metadata = {
  name: 'Clutch The Bald Eagle Staking',
  description: 'Stake CLUTCH, World Cup rewards',
      ...
      {projectedReturns && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            ...
            <span style={{ fontFamily: 'Bebas Neue, sans-serif' }}>{projectedReturns.principal.toFixed(2)} CLUTCH</span>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            <span style={{ color: '#6b7280' }}>Rewards ({projectedReturns.days} days):</span>
            <span style={{ fontFamily: 'Bebas Neue, sans-serif', color: '#059669' }}>
              +{projectedReturns.reward.toFixed(2)} CLUTCH
            </span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ color: '#6b7280', fontWeight: 'bold' }}>Total:</span>
            <span style={{ fontFamily: 'Bebas Neue, sans-serif', fontWeight: 'bold', fontSize: '1.25rem' }}>
              {projectedReturns.total.toFixed(2)} CLUTCH
            </span>
          </div>
        </div>
      )}
...
const AppContent = () => {
  ...
  const fetchQuote = async () => {
    ...
    try {
      const text = ' World Cup Token- CLUTCH THE BALD EAGLE ';
      setApiQuote(text);
    } finally {
  ...
  return (
    <div className="app-container" style={{ backgroundImage: `url('data:image/svg+xml;base64,${btoa(backgroundPattern)}')` }}>
      <style>{`
        ...
        .app-container { min-height: 100vh; padding: 1rem; display: flex; flex-direction: column; align-items: center; background-color: var(--bg-light); color: var(--primary-color); font-family: 'Inter', sans-serif; background-size: 24px 24px; }
        ...
        .app-title { font-size: 1.875rem; font-family: 'Bebas Neue', sans-serif; color: var(--primary-color); text-shadow: 2px 2px 4px rgba(0,0,0,0.1); letter-spacing: 0.05em; margin-left: 1rem; }

        .connect-button { position: relative; display: inline-flex; height: 3rem; width: 100%; align-items: center; justify-content: center; border-radius: 9999px; padding-left: 1.5rem; padding-right: 1.5rem; font-family: 'Bebas Neue', sans-serif; color: white; transition: all .3s; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); background: linear-gradient(to right, #fb923c, #ea580c); }
        .connect-button:hover { transform: scale(1.05); background: linear-gradient(to right, #fb923c, #f97316); }
        ...
        .disconnect-button { padding: 0.25rem; border-radius: 9999px; color: #6b7280; transition: color .3s; margin-left: 0.5rem; }
        ...
        .stats-section h2 { font-size: 1.5rem; font-family: 'Bebas Neue', sans-serif; margin-bottom: 1rem; display: flex; align-items: center; }
        ...
        .stat-card p.label { font-size: 0.875rem; color: #6b7280; }
        .stat-card p.value { font-size: 1.875rem; font-weight: 800; font-family: 'Bebas Neue', sans-serif; margin-top: 0.25rem; }
        ...
        .pool-card h2 { font-size: 1.5rem; font-family: 'Bebas Neue', sans-serif; color: var(--primary-color); margin-bottom: .5rem; display: flex; align-items: center; position: relative; z-index: 1; }
        .pool-card .apy { font-size: 1.875rem; font-family: 'Bebas Neue', sans-serif; color: var(--secondary-color); margin-bottom: 1rem; position: relative; z-index: 1; }
        .input-group { position: relative; z-index: 1; }
        .input-group p { font-size: .875rem; color: #6b7280; margin-bottom: .5rem; }
        .input-flex { display: flex; gap: .5rem; }
        .input-field { flex-grow: 1; background-color: white; color: var(--primary-color); padding: .75rem; border-radius: .75rem; border: 2px solid var(--border-color); transition: all .3s; font-family: 'Bebas Neue', sans-serif; font-size: 1.125rem; }
        .input-field:focus { outline: none; }
        .stake-button { padding: .75rem 1.5rem; border-radius: .75rem; font-family: 'Bebas Neue', sans-serif; color: white; background-color: var(--primary-color); transition: transform .2s, background-color .2s; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); }
        ...
        .unstake-button { padding: .75rem 1.5rem; border-radius: .75rem; font-family: 'Bebas Neue', sans-serif; color: white; background-color: #6b7280; transition: transform .2s, background-color .2s; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); }
        ...
        .claim-section h2 { font-size: 1.5rem; font-family: 'Bebas Neue', sans-serif; color: var(--primary-color); margin-bottom: 1rem; display: flex; align-items: center; }
        .claim-button { width: 100%; display: flex; align-items: center; justify-content: center; padding: .75rem; border-radius: .75rem; font-family: 'Bebas Neue', sans-serif; font-size: 1.125rem; transition: all .3s; background-color: var(--success-color); color: white; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); }
        ...
        .welcome-title { font-size: 3rem; font-family: 'Bebas Neue', sans-serif; color: var(--primary-color); margin-bottom: 1rem; text-shadow: 2px 2px 4px rgba(0,0,0,0.1); }
        .welcome-message { color: #4b5563; margin-bottom: 2rem; max-width: 28rem; }
        ...
        .modal-title { font-size: 1.5rem; font-weight: 700; font-family: 'Bebas Neue', sans-serif; margin-bottom: .5rem; }
        .modal-text { font-family: 'Inter', sans-serif; }
      ...
      <div className="main-wrapper">
        ...
        <header className="header">
          <div className="logo-section">
            <img src={logoUrl} alt="CLUTCH THE BALD EAGLE" className="logo-image animate-pulse" />
          </div>
        ...
        {isConnected ? (
          <main>
            ...
            <div className="stats-section">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h2><Cat style={{ marginRight: '0.5rem' }} /> Your CLUTCH Staking Stats</h2>
                {isRefreshing && (
              ...
              <div className="stats-grid">
                <div className="stat-card">
                  ...
                  <p style={{ fontSize: '0.75rem', fontFamily: 'Inter, sans-serif', color: '#4b5563', marginTop: '0.25rem' }}>CLUTCH Tokens</p>
                </div>
                <div className="stat-card">
                ...
                <div className="stat-card">
                  ...
                  <p style={{ fontSize: '0.75rem', fontFamily: 'Inter, sans-serif', color: '#4b5563', marginTop: '0.25rem' }}>CLUTCH Tokens (sum of pools)</p>
                </div>
              </div>
              ...
              <div className="stats-grid" style={{ marginTop: '1rem' }}>
                <div className="stat-card">
                  ...
                  <p style={{ fontSize: '0.75rem', fontFamily: 'Inter, sans-serif', color: '#4b5563', marginTop: '0.25rem' }}>
                    CLUTCH Available for Rewards
                  </p>
                </div>
                <div className="stat-card">
                  <p className="label">Total Staked (All Users)</p>
                  <p className="value">{formattedTotalStakedAll.toFixed(2)}</p>
                  <p style={{ fontSize: '0.75rem', fontFamily: 'Inter, sans-serif', color: '#4b5563', marginTop: '0.25rem' }}>CLUTCH in contract</p>
                </div>
                <div className="stat-card">
            ...
            <div className="claim-section">
              ...
              <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
                <p style={{ fontSize: '1.5rem', fontWeight: 'bold', fontFamily: 'Bebas Neue, sans-serif' }}>{totalUserRewards.toFixed(6)} CLUTCH</p>
          ...
          <div className="welcome-message-container">
            <h1 className="welcome-title">Welcome to Clutch Staking!</h1>
            <p className="welcome-message">Connect your wallet to start staking your CLUTCH tokens and earn rewards. It's time to put your tokens to work!</p>
            <button onClick={() => open()} className="connect-button">
              <span className="connect-button-content"><Wallet className="icon-mr-2 h-5-w-5" />Connect Wallet</span>
            </button>
            <button
        ...
        {showModal && (
          <div className="modal-overlay">
            <div className={`modal-container ${modalType === 'success' ? 'success' : 'error'}`}>
              ...
              <div>
                <h3 className="modal-title">{modalType === 'success' ? 'CLUTCH-nificent!' : 'Oh no!'}</h3>
